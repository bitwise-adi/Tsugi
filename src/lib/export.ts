...
export async function exportAsJSON(): Promise<void> {
  ...
  downloadFile(JSON.stringify(data, null, 2), `tsugit-backup-${timestamp}.json`, 'application/json');
}
...
export async function exportAsCSV(): Promise<void> {
  ...
  downloadFile(sections, `tsugit-export-${timestamp}.csv`, 'text/csv');
}
